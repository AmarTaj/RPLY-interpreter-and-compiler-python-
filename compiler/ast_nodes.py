...
class ExprFuncDef(ExprNode):
    """
    children[0]: Identifier
    children[1]: Arguments
    children[2]: Return type
    children[3]: Function Body
    """ 
    def compile(self, st, output):
        name = self.children[0]
        args = self.children[1]
        ret_type = self.children[2]
        func_body = self.children[3]

        mem_ret_label = st.declare(name,'label')
        label_start, label_end, label_return = st.next_func_labels(name, ret_type)

        output += [f'JUMP {label_end}']
        output += [f'{label_start}:']
        output += [f'POP {mem_ret_label}']

        if args is not None:
            for arg in args:
                arg_label = st.alloc(arg[0].value)
                output += [f'POP {arg_label}']

        if func_body is not None:
            for i in func_body:
                i.compile(st, output)

        output += [f'JUMP {mem_ret_label}']
        output += [f'{label_end}:']

        return mem_ret_label

class ExprFuncCall(ExprNode):
    """
    children[1]: Function name
    children[2]: Arguments
    """
    def compile(self, st, output):
        func = self.children[0]
        ret_label = st.get_label(func)
        mem = st.lookup(func)

        result_type = st.get_type(func)
        result = st.alloc(result_type)

        output += [f'PUSH {mem}']
        output += [f'PUSH {ret_label}']
        output += [f'JUMP function-begin-{func}']
        output += [f'{ret_label}:']
        output += [f'POP {result}']

        return result

class RetNode(ExprNode):
    """
    children[0]: return expr
    """
    def compile(self, st, output):
        label = st.label_for_return()
        expr = self.children[0].compile(st, output)
        output += [f'PUSH {expr}']
        output += [f'JUMP {label}']
