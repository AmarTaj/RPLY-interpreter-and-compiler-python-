...
#     def __init__(self, args = [], func_label = None, ret_type = None):
#         self.args = args
...
class CompilerSymbolTable():
    ...
    def __init__(self):
        ...
        self.func_identifiers = ChainMap()  # Identifier to functions
        self.memory = {}
    ...
    def next_func_labels(self, name, ret_type):
        '''
        Return the next labels for functions
        '''
        label_start = 'function-begin-' + name
        label_end = 'function-end-' + name
        label_return = 'function-return-' + str(self.next_labels['function-return'].next())

        self.func_identifiers[name] = [label_return, ret_type]

        return label_start, label_end, label_return

    def get_label(self, name):
        '''
        Return label
        '''
        return self.func_identifiers[name][0]

    def get_type(self, name):
        '''
        Return type
        '''
        return self.func_identifiers[name][1]

    def label_for_return(self):
        '''
        label for return node
        '''
        l = list(self.var_identifiers.keys())
        return self.func_identifiers[l[-1]][0]
    ...
    #         arg_entry = self.declare(arg_name, arg_type)
    #         arg_entries.append(arg_entry)

    #     self.var_identifiers[func_name] = FunctionEntry(arg_entries, func_label, returnType)
