...
def build_parser(possible_tokens):
    ...
    @pg.production('param_list : PAREN_CLOSE')
    @pg.production('param_list : param_list_head param_list_tail PAREN_CLOSE')
    def param_list(p):
        if len(p)>1:
            elist = p[0] + p[1]
            return [elist]

    @pg.production('param_list_head : SCALAR_TYPE COLON IDENTIFIER')
    @pg.production('param_list_head : ARRAY_TYPE BRACKET_CLOSE COLON IDENTIFIER')
    def param_list_head(p):
        return [p[0]]

    @pg.production('param_list_tail : ')
    @pg.production('param_list_tail : COMMA param_list_head param_list_tail')
    def param_list_tail(p):
        if len(p) == 0:
            return []
        else:
            return [p[1]] + p[2]
    ...
    @pg.production('expr : ')
    @pg.production('expr : RETURN expr')
    def return_def(p):
        if len(p)>0:
            return RetNode([p[1]])
        else:
    ...
    @pg.production('expr : FUNC param_list')
    def func_call(p):
        return ExprFuncCall([p[0].value[:-1]])

    @pg.production('expr : DEF FUNC param_list POINT SCALAR_TYPE COLON cmd_list END')
    @pg.production('expr : DEF FUNC param_list POINT ARRAY_TYPE BRACKET_CLOSE COLON cmd_list END')
    def fun_def(p):
        if len(p)==8:
            return ExprFuncDef([p[1].value[:-1], p[2], p[4].value, p[6]])
        elif len(p)==9:
            return ExprFuncDef([p[1].value[:-1], p[2], p[4].value, p[7]])
